...
class TestGetConstrainedDofIndices:
    """Tests for get_constrained_dof_indices constraint identification."""
    def test_fixed_u_on_bar_returns_one_index(self) -> None:
        """FIXED_U on bar returns one constrained DOF."""
    ...
    def test_fixed_all_on_beam_returns_two_indices(self) -> None:
        """FIXED_ALL on beam returns two constrained DOFs."""
    ...
    def test_pin_on_beam_returns_v_only(self) -> None:
        """PIN on beam constrains V only, not THETA."""
    ...
    def test_roller_constrains_v_only(self) -> None:
        """ROLLER constrains V only."""
    ...
    def test_constrained_indices_are_sorted(self) -> None:
        """Constrained DOF indices are sorted."""
        model = make_beam_model_with_bc(BoundaryConditionType.FIXED_ALL)
        dof_map = make_beam_dof_map(2)
        constrained = get_constrained_dof_indices(model, dof_map)
...
class TestGetFreeDofIndices:
    """Tests for get_free_dof_indices free DOF identification."""
    def test_free_plus_constrained_equals_all(self) -> None:
        """Free DOFs plus constrained DOFs equal all DOFs."""
        model = make_bar_model_with_bc(BoundaryConditionType.FIXED_U)
        dof_map = make_bar_dof_map(2)
    ...
    def test_no_overlap_between_free_and_constrained(self) -> None:
        """Free and constrained DOFs have no overlap."""
        model = make_beam_model_with_bc(BoundaryConditionType.FIXED_ALL)
        dof_map = make_beam_dof_map(2)
    ...
    def test_free_indices_are_sorted(self) -> None:
        """Free DOF indices are sorted."""
        model = make_bar_model_with_bc(BoundaryConditionType.FIXED_U)
        dof_map = make_bar_dof_map(2)
        free = get_free_dof_indices(model, dof_map)
...
class TestApplyConstraintsReduction:
    """Tests for apply_constraints_reduction matrix reduction."""
    def test_reduced_K_shape(self) -> None:
        """Reduced stiffness matrix has correct shape."""
    ...
    def test_reduced_K_correct_values(self) -> None:
        """Reduced stiffness matrix has correct values."""
    ...
    def test_reduced_F_correct_values(self) -> None:
        """Reduced force vector has correct values."""
        K = np.array([[1,-1,0],[-1,2,-1],[0,-1,1]], dtype=float)
        F = np.array([0.0, 0.0, 5.0])
        K_ff, F_f = apply_constraints_reduction(K, F, constrained_dofs=[0])
        np.testing.assert_allclose(F_f, np.array([0.0, 5.0]))

    def test_reduced_K_is_symmetric(self) -> None:
        """Reduced stiffness matrix is symmetric."""
