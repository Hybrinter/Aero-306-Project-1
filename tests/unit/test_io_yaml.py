...
class TestLoadModelFromYaml:
    def test_returns_fea_model(self, bar_yaml_path: Path):
        from fea_solver.models import FEAModel
        model = load_model_from_yaml(bar_yaml_path)
        assert isinstance(model, FEAModel)

    def test_label_parsed(self, bar_yaml_path: Path):
        model = load_model_from_yaml(bar_yaml_path)
        assert model.label == "test_bar"

    def test_nodes_count(self, bar_yaml_path: Path):
        model = load_model_from_yaml(bar_yaml_path)
        assert len(model.mesh.nodes) == 2

    def test_node_ids_and_coords(self, bar_yaml_path: Path):
        model = load_model_from_yaml(bar_yaml_path)
    ...
    def test_nodal_load_parsed(self, bar_yaml_path: Path):
        model = load_model_from_yaml(bar_yaml_path)
    ...
    def test_element_node_references_resolved(self, bar_yaml_path: Path):
